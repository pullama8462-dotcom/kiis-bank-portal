...
export default function SecurityLabSection({ className = '' }) {
  ...
  const checkLabStatus = useCallback(async () => {
    ...
    setErrorMsg('')
    try {
      const res = await fetch(`${BACKEND_URL}/api/lab/status`)
      if (res.ok) {
  ...
  const forgeToken = async (kind) => {
    ...
    try {
      ...
      const res = await fetch(
        `${BACKEND_URL}/api/lab/forge-token/${kind}?${params.toString()}`
      )
      if (!res.ok) {
        ...
        {labOn === true && (
          <>
            ...
            <button
              ...
              className="mt-5 w-full pulse-glow-cyan bg-cyber-cyan hover:bg-cyan-400 text-cyber-dark font-bold font-mono text-xs py-3 rounded-lg tracking-[2px] transition-all disabled:opacity-40"
            >
              {testLoading ? 'Calling API…' : 'Test admin API — GET /api/admin/users'}
            </button>
